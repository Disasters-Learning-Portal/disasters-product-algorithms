...
- **Guard rails:** `activation_event` default `YYYYMM_Example_Event` is REJECTED by run.sh (forces a real event); `source_label` is required (no default); `dst_crs` defaults to `native` (EPSG:3857 is NOT required for `build_stac`).
