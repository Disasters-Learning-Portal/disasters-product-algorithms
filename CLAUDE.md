...
- **`cog_metadata.create_cog_with_metadata` in-memory (bytes) path returns a `bytearray`, and defaults `web_optimized=True` (which reprojects to EPSG:3857).** Any bytes→bytes caller (e.g. an S3 download→bake→upload loop) must (a) pass `web_optimized=False, target_crs=None, preserve_compression=True` to keep the COG identical, and (b) wrap the result in `bytes(...)` before handing it to `rasterio.io.MemoryFile` or boto3 `put_object` — `MemoryFile` rejects a `bytearray` with `TypeError: Constructor argument must be a file opened in binary mode or bytes`. Validate the bytes locally with `validate_cog_in_memory` / `rio_cogeo.cog_validate`: the VEDA raster API at `dev.disasters.openveda.cloud/api/raster` is a **titiler-pgstac mosaic** endpoint (`/searches/...` only; **no `/cog/validate`**), so it can't validate an arbitrary local/S3 COG — local `cog_validate` is the same check titiler wraps.
