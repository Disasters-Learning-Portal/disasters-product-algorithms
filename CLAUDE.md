...
- **Registration schema is OGC/CWL**, not the legacy maap-py format. `algorithm_name` must match `^[a-z0-9_-]+$` (lowercase, no spaces/slashes — e.g. `landsat-8-9`). Use **`base_container_url`** (NOT `docker_container_url`) = `mas.maap-project.org/root/maap-workspaces/custom_images/maap_base:v4.2.0`. **`build_command`/`run_command` MUST be prefixed with the repo dir name** (`disasters-product-algorithms/dps/<sensor>/...`) — MAAP clones to `/app/<repo>/` and runs from `/app`. Resources are `ram_min`/`cores_min`/`outdir_max` (not queue/disk_space). `inputs` is a flat list `{name,label,doc,type,default}`; valid types = string/int/File/Directory/long/float/boolean/double (no enum/array); toggles = `boolean`. **CRITICAL: every defaulted/optional input MUST carry the `?` type-suffix (`boolean?`/`string?`/`int?`/`float?`)** — a `default:` alone does NOT set OGC `optional`, and the MAAP **Submit Job** form (`dps-jupyter-extension/SubmitJob.tsx`: `value = formInputs[key] || null; if (!input.optional && value == null) → "Valid value required."`) blocks submission for any falsy value (`false`/`""`/`0`) on a non-`optional` input. So a `boolean` defaulting `false` or a `string` `default: ""` jams the form until its type is `?`. OGC keeps `type` and `optional` separate, so `boolean?` still renders a toggle. Rule: **has a `default:` → give it `?`**; keep `?` OFF genuinely-required inputs (`file_path_of_raw_data`, SAR `date`, `source_label`, `activation_event`).
