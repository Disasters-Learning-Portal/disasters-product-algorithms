...
set -euo pipefail
# MAAP DPS build script (Landsat). Run ONCE per worker image build.
# Creates the shared `disasters_dps` conda env and installs this repo so the
# `process_landsat89` console script is on PATH for run.sh's `conda run`.

# basedir = dps/landsat/ ; repo_root = two levels up.
