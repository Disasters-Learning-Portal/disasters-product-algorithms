...
set -euo pipefail
# MAAP DPS build script (list-dates discovery). Run ONCE per worker image build.
# Creates the shared `disasters_dps` conda env and installs this repo so the
# process_capella / process_umbra / process_satellogic console scripts are on
# PATH for run.sh's `conda run` dispatch.

# basedir = dps/list_dates/ ; repo_root = two levels up.
