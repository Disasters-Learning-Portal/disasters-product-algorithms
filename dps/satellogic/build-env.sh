...
set -euo pipefail
# MAAP DPS build script (Satellogic). Run ONCE per worker image build.
# Creates the shared `disasters_dps` conda env and installs this repo so the
# `process_satellogic` console script is on PATH for run.sh's `conda run`.

# basedir = dps/satellogic/ ; repo_root = two levels up.
