...
set -euo pipefail
# MAAP DPS build script (Sentinel-2). Run ONCE per worker image build.
# Creates the shared `disasters_dps` conda env and installs this repo so the
# `process_sentinel2` console script is on PATH for run.sh's `conda run`.

# basedir = dps/sentinel2/ ; repo_root = two levels up.
