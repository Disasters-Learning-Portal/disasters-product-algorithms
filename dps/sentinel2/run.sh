...
set -euo pipefail
# MAAP DPS run script (Sentinel-2). Called by DPS per job.
#
# DPS contract:
...
# --- Run the processor ---
# process_sentinel2 takes the input dir positionally, unpacks to
# input/unpacked/, and writes product COGs to input/output/ (nested date/product/).
# shellcheck disable=SC2086  # intentional word-split of PRODUCTS into nargs='*'
conda run --live-stream --name disasters_dps \
  process_sentinel2 "${INPUT_DIR}" \
    -p ${PRODUCTS} \
    -dst_crs "${DST_CRS}" \
...
# The CLI wrote COGs to input/output/ (process_sentinel2.py:257). Without this
# copy the job "succeeds" but uploads nothing. cp -r preserves the date/product
# subdir structure.
