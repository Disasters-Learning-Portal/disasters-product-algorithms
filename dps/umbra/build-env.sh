...
set -euo pipefail
# MAAP DPS build script (Umbra). Run ONCE per worker image build.
# Creates the shared `disasters_dps` conda env and installs this repo so the
# `process_umbra` console script is on PATH for run.sh's `conda run`.

# basedir = dps/umbra/ ; repo_root = two levels up.
