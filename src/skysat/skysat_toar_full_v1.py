...
IN_PATH        = "/mnt/disasters1/data/esops/eventData/2026/HI_Flooding_March/skysat/20260323_173545/SkySatCollect"
TOAR_IMAGE_PATH= os.path.join(IN_PATH, "20260323_173545_ssc4_u0001_analytic.tif")          
UDM_PATH       = os.path.join(IN_PATH, "20260323_173545_ssc4_u0001_udm2.tif") 
OUTPUT_DIR     = "output_toar"                

# Optional Processing Toggles
USE_UDM        = True   # Set to False to skip UDM masking
CALC_NDVI      = True   # Set to False to skip NDVI calculation & export
CALC_EVI       = True   # Set to False to skip EVI calculation & export
CALC_NDWI      = True   # Set to False to skip NDWI calculation & export

# SkySat TOAR band indices (1-based) standard order
BLUE_BAND  = 1  
GREEN_BAND = 2
RED_BAND   = 3  
NIR_BAND   = 4
...
# ══════════════════════════════════════════════════════
# STEP 1: LOAD TOAR BANDS & CONVERT TO TOA REFLECTANCE
# ══════════════════════════════════════════════════════
print("\n── Step 1: Loading TOAR bands & converting to Reflectance ──")
REFLECTANCE_COEFFS = {}

with rasterio.open(TOAR_IMAGE_PATH) as src:
    profile = src.profile.copy()
    dataset_tags = src.tags()
    
    image_desc = dataset_tags.get('ImageDescription') or dataset_tags.get('TIFFTAG_IMAGEDESCRIPTION')
    ...
    try:
        ...
        # Map the list to bands 1, 2, 3, 4
        REFLECTANCE_COEFFS[1] = coeffs_list[0]
        REFLECTANCE_COEFFS[2] = coeffs_list[1]
        REFLECTANCE_COEFFS[3] = coeffs_list[2]
        REFLECTANCE_COEFFS[4] = coeffs_list[3]
        
        print(f"  ✔ Extracted reflectance coefficients: {REFLECTANCE_COEFFS}")
...
# Scale DN to TOA Reflectance
blue  = np.clip(blue_dn  * REFLECTANCE_COEFFS[BLUE_BAND], 0, 1)
green = np.clip(green_dn * REFLECTANCE_COEFFS[GREEN_BAND], 0, 1)
red   = np.clip(red_dn   * REFLECTANCE_COEFFS[RED_BAND], 0, 1)
nir   = np.clip(nir_dn   * REFLECTANCE_COEFFS[NIR_BAND], 0, 1)
print(f"  Loaded & converted {4} bands from: {TOAR_IMAGE_PATH}")
...
write_rgb_uint8(
    os.path.join(IN_PATH, OUTPUT_DIR, "skysat_toar_TrueColor2.tif"),
    r_band=red, g_band=green, b_band=blue, profile=profile,
    tags=dict(DESCRIPTION=f"True Color RGB (24-bit) — SkySat TOA (Gamma {RGB_GAMMA})", UDM_APPLIED=udm_applied),
    gamma=RGB_GAMMA
)

write_rgb_uint8(
    os.path.join(IN_PATH, OUTPUT_DIR, "skysat_toar_CIR2.tif"),
    r_band=nir, g_band=red, b_band=green, profile=profile,
    tags=dict(DESCRIPTION=f"Color Infrared CIR (24-bit) — SkySat TOA (Gamma {RGB_GAMMA})", UDM_APPLIED=udm_applied),
    gamma=RGB_GAMMA
)

if CALC_NDVI:
    write_geotiff(
        os.path.join(IN_PATH, OUTPUT_DIR, "skysat_toar_NDVI.tif"), 
        ndvi, profile, nodata=NODATA_VALUE, 
        tags=dict(DESCRIPTION="NDVI — SkySat TOA", UDM_APPLIED=udm_applied)
    )
if CALC_EVI:
    write_geotiff(
        os.path.join(IN_PATH, OUTPUT_DIR, "skysat_toar_EVI.tif"), 
        evi, profile, nodata=NODATA_VALUE, 
        tags=dict(DESCRIPTION="EVI — SkySat TOA", UDM_APPLIED=udm_applied)
    )
if CALC_NDWI:
    write_geotiff(
        os.path.join(IN_PATH, OUTPUT_DIR, "skysat_toar_NDWI.tif"), 
        ndwi, profile, nodata=NODATA_VALUE, 
        tags=dict(DESCRIPTION="NDWI — SkySat TOA", UDM_APPLIED=udm_applied)
    )
...
output_png = os.path.join(IN_PATH, OUTPUT_DIR, "skysat_toar_dashboard.png")

fig = plt.figure(figsize=(24, 14))
fig.suptitle(f"SkySat Derived Products Dashboard\nTOA Reflectance — True Color (Gamma {RGB_GAMMA}) | CIR | NDVI | EVI | NDWI", fontsize=15, fontweight='bold', y=0.98)
...
print("\n✅ All TOA products & COGs complete!")
