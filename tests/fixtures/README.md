...
| `dswx_chngmap_float32_classcodes_crop.tif` | 256², 1x **float32**, EPSG:3857, nodata -9999 | OPERA DSWx-S1 BWTR **change map**: valid values are exactly `{-1, 0, +1}` in a FLOAT band. The one fixture that disproves "float means continuous" — cropped from the genuine 16384x33792 product, not synthesised. Keeps nodata pixels so it also exercises the audit's nodata exclusion |
